...
public:
	...
	int modify_profile(char *msg = NULL) {
		scanner buffer;
		char *tmsg = msg;
		buffer.set(tmsg);
		int id = buffer.geti();
	...
	string add_date(string ta, int tb) {
		string ret = ta;
		if (tb) {
			if (ret[8] == '3' && ret[9] == '0') {
                                ret[8] = '0'; ret[9] = '1';
	...
	char *query_transfer(char *msg = NULL) {
		scanner buffer;
		char *tmsg = msg;
		buffer.set(tmsg);
		tstring<20> loc1;
	...
	char *query_order(char *msg = NULL) {
		char *tmsg = msg;
		scanner buffer;
		buffer.set(tmsg);
		tstring<20> uid;
	...
	int refund_ticket(char *msg = NULL) {
		char *tmsg = msg;
		scanner buffer;
		buffer.set(tmsg);
		tstring<20> uid;
